...
// ✅ CORRECTION 1 : Recréer __dirname pour les ES Modules
...
pool.connect(async (err, client, release) => {
  ...
  try {
    // ✅ Création de la table pour le nouveau formulaire Booking
    await client.query(`
    ...
    // Sécurité : On s'assure que toutes les colonnes nécessaires existent pour les rendez-vous
    const columnsToAdd = [
    ...
    for (const query of columnsToAdd) {
      // On utilise .catch() individuellement pour ne pas bloquer si une colonne existe déjà
    ...
    // On libère le client dans tous les cas
    release();
  }
});
...
async function startServer() {
  ...
  const PORT = 3000;
    ...
    // ✅ Autoriser la lecture des images uploadées depuis le frontend
  ...
  // ✅ NOUVEAU MIDDLEWARE : Vérifie si on est connecté pour les routes privées
  ...
  app.use(cors()); // ✅ Active le CORS ici aussi !


  // 2. Route pour que ton interface Admin lise les demandes
  app.get("/api/requests", requireAuth, async (req, res) => {
    const result = await pool.query(`SELECT * FROM booking_requests ORDER BY created_at DESC`);
    res.json(result.rows);
  });

  // 3. Route pour changer le statut (Accepter/Refuser)
  app.put("/api/requests/:id/status", requireAuth, async (req, res) => {
    const { id } = req.params;
    const { status } = req.body;
    await pool.query(`UPDATE booking_requests SET status = $1 WHERE id = $2`, [status, id]);
  ...
  app.post("/api/requests", publicUpload.array('images', 3), async (req: any, res) => {
    try {
      ...
      if (!clientName || !clientEmail || !projectDescription) {
        return res.status(400).json({ error: "Les champs Nom, Email et Description sont obligatoires." });
      }

      const imageUrls = req.files ? (req.files as Express.Multer.File[]).map(f => `/uploads/${f.filename}`) : [];

      const result = await pool.query(
        `INSERT INTO booking_requests 
        (client_name, client_email, client_phone, instagram, project_description, placement, estimated_size, availability_prefs, reference_images, status) 
      ...
      res.status(500).json({ error: "Erreur serveur lors de la sauvegarde de la demande." });
    }
  });
  ...
  app.put("/api/requests/:id/status", requireAuth, express.json(), async (req, res) => {
    try {
      ...
      if (result.rowCount === 0) return res.status(404).json({ error: "Demande introuvable." });
