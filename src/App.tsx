...
const AppointmentDetailView = ({ appointment, onBack, onUpdate, apiFetch }: any) => {
  ...
  const checkConsentStatus = async () => {
    try {
      const response = await apiFetch(`/api/appointments/${appointment.id}/check-consent`);
      if (response.ok) {
        const data = await response.json();
        setConsentExists(data.exists);
      }
    } catch (err) {
      console.error("Erreur check consent:", err);
    }
  };
  ...
  return (
    <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-8">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center space-x-3">
          {!isEditing ? (
            <button onClick={() => setIsEditing(true)} className="px-4 py-2 bg-white/5 hover:bg-white/10 border border-white/10 rounded-xl text-sm font-medium flex items-center space-x-2">
              <Edit2 size={16} />
              <span>Modifier</span>
            </button>
          ) : (
            <>
              <button onClick={() => setIsEditing(false)} className="px-4 py-2 text-gray-400 hover:text-white text-sm">Annuler</button>
              <button onClick={handleSave} disabled={saving} className="px-4 py-2 bg-lilas text-black rounded-xl text-sm font-bold flex items-center space-x-2">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        {/* Colonne Gauche : Infos Client & Projet */}
          ...
          <div className="glass-card p-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              <div className="space-y-1">
                ...
                {isEditing ? (
                  <input type="text" value={formData.location} onChange={(e) => setFormData({...formData, location: e.target.value})} className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3" />
                ) : (
                  <p className="text-lg font-medium">{formData.location || 'Non renseigné'}</p>
                )}
              </div>
              <div className="space-y-1">
                <label className="text-[10px] uppercase text-gray-500 font-bold tracking-widest">Taille</label>
                {isEditing ? (
                  <input type="text" value={formData.size} onChange={(e) => setFormData({...formData, size: e.target.value})} className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3" />
                ) : (
                  <p className="text-lg font-medium">{formData.size || 'Non renseignée'}</p>
                )}
              </div>
            </div>
          </div>
        ...
        {/* Colonne Droite : Paiement & Communication */}
        <div className="space-y-8">
          <div className="glass-card p-8 border-t-4 border-lilas">
            <h3 className="text-xl font-bold mb-6 flex items-center space-x-2 text-lilas">
              <Wallet size={20} />
              <span>Paiement</span>
            </h3>
            <div className="space-y-4">
               ...
               <div className="flex justify-between pt-4 border-t border-white/5">
                 <span className="text-gray-300">Reste à percevoir</span>
          ...
          <div className="glass-card p-6 border-t-4 border-blue-500/50">
            ...
            <button
              onClick={() => window.open(`/api/appointments/${appointment.id}/download-consent`, '_blank')}
              className="w-full flex items-center justify-center space-x-2 py-3 px-4 bg-blue-600/10 text-blue-400 border border-blue-500/20 rounded-xl text-sm font-bold hover:bg-blue-600 hover:text-white transition-all mb-3"
            >
              <Download size={18} />
              <span>Télécharger la fiche</span>
            </button>

            {/* BOUTON ENVOI FICHE DE SOINS */}
            <button 
              onClick={handleSendPdf}
              className={`w-full py-3 border rounded-xl text-sm font-bold transition-all flex items-center justify-center space-x-2
              ...
              `}
            >
               {emailStatus === 'sending' ? <RefreshCw size={18} className="animate-spin" /> : <Mail size={18} />}
               <span>{emailStatus === 'success' ? 'Email envoyé !' : 'Envoi fiche de soins'}</span>
            </button>
  ...
  />
          ...
          <div className="glass-card p-6 bg-rose-500/5 border border-rose-500/10">
            <button onClick={() => setShowDeleteConfirm(true)} className="w-full flex items-center justify-center space-x-2 text-rose-400 py-2">
              <Trash2 size={18} />
